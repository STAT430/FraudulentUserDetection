...
public:
    ...
    unordered_map<string, double> test_error(){
        ...
        result["FNR"] = fn / (fn + tp);
        result["TNR"] = tn / (fp + tn);
        result["Precison"] = tp / (tp + fp);
        result["Recall"] = result["TPR"];
        return result;
        
    }
    
    unordered_map<string, double> test_error_alter(){
        ...
        for (iter = pos_test_set.begin(); iter != pos_test_set.end(); iter++) {
            ...
            if (post[*iter] >= thresh_pos || post[*iter] <= thresh_neg) {
                label_vec.push_back(1);
                predictions.push_back(post[*iter]);
            }
        ...
        for (iter = pos_test_set.begin(); iter != pos_test_set.end(); iter++) {
            ...
            if (post[*iter] >= thresh_pos){
                tp += 1;
            }
            if (post[*iter] <= thresh_neg){
                fn += 1;
            }
        ...
        result["Predicted_P"] = (fp + tp) / (pos_test_set.size() + neg_test_set.size());
        result["ACC_P"] = tp / (fp + tp);
        result["Predicted_N"] = (fn + tn) / (pos_test_set.size() + neg_test_set.size());
        result["ACC_N"] = tn / (fn + tn);

        // Add Predicted_FPR, Predicted_RNR
        result["FPR"] = fp / (fp + tn);
        result["FNR"] = fn / (tp + fn);
        result["TPR"] = tp / (tp + fn);
        result["TNR"] = tn / (fp + tn);
    ...
    void parse_par(int argc, char **argv){
        ...
        while (i < argc) {
            if (strcmp(argv[i],"-t") == 0){
                ...
                thresh_pos = (double)atof(argv[i + 1]);
                type = 1;
            }
            else if (strcmp(argv[i],"-tn") == 0){
                thresh_neg = (double)atof(argv[i + 1]);
                type = 1;
            }
